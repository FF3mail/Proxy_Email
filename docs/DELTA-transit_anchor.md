...
| **Inbound attach gate (79.2d/e/i)** | One path for N≥1; attachment = Disposition contains `attachment` only (inline ignored). Archives ∪ images (no SVG); MAX=20; archive Subject (E2 NFC); `missing_filename` skip; **`nested_message`** for `message/rfc822` attachment (opaque, no inner walk); dispose order Seen→delete (E4); shared enumerator with `nested_policy` (inbound opaque / outbound error). Reports: `PROMPT-79-2d-…`, `PROMPT-79-2e-…`, `PROMPT-79-2i-….md` |
