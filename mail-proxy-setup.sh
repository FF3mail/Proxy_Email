...
install -d -m 2750 -o vmail -g mail-proxy-logs /var/log/mail-proxy
if [[ -f ./tmpfiles.d-mail-proxy.conf ]]; then
    install -m 0644 -o root -g root ./tmpfiles.d-mail-proxy.conf /etc/tmpfiles.d/mail-proxy.conf
    systemd-tmpfiles --create /etc/tmpfiles.d/mail-proxy.conf
else
    touch /var/log/mail-proxy/mail-proxy-daemon.log
    chown vmail:mail-proxy-logs /var/log/mail-proxy/mail-proxy-daemon.log
    chmod 0640 /var/log/mail-proxy/mail-proxy-daemon.log
